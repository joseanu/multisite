        ...
        <div class="productos">
          <div id="filtro" class="productosNav" v-if="filtros.length > 1">
            ...
            <button
              type="button"
              class="boton blanco"
              v-for="tipo in filtros"
              :key="tipo.slug"
              @click="filtrar(tipo.slug)">{{ tipo.nombre }}</button>
          </div>
          <transition-group name="list" tag="div" class="productosLista">
            <template v-for="(listado, tipo) in productos">
              <producto
                v-for="producto in listado"
                :tipo="tipo"
                :producto="producto"
                :key="producto.modelo"></producto>
            </template>
          </transition-group>
        </div>
      </div>
  ...
  computed: {
    ...
    productos() {
      ...
      if (this.parametro) {
        ...
        return subset(this.filtrado)(this.$store.getters.getProductosBySlug(this.parametro));
      }
      return {};
    },
    filtros() {
